...
# 
# Ищет оптимальные параметры, минимизирующие пик заболеваемости и долю умерших.
...
function cost_multi(x)
    replicates = 5   # ноутбук не тянет, уменьшил с 5 до 3
    peak_vals = Float64[]
    dead_vals = Int[]
    
    for rep = 1:replicates
        model = initialize_sir(;
            Ns = [1000, 1000, 1000],   # ноутбук не тянет, уменьшил с 1000 на 500
            β_und = fill(x[1], 3),
            β_det = fill(x[1]/10, 3),
            infection_period = 14,
