# # Агентная модель SIR (src/sir_model.jl)
...
# - 'days_infected': количество дней с момента заражения (для инфицированных)
# - 'status': текущее состояние ('S' - восприимчивый, 'I' - зараженный, 'R' - переболевший)
...
# Создаёт модель, населяет города агентами и задаёт начальное число инфециорованных
...
# ## Шаг агента (Выполняется для каждого агента на каждом шаге)
# Определяет поведение агента: миграции, передача инфекции обновление счётчика, выздоровление/смерть
...
# Агент переместиться в другой город, с вероятностью заданной в матрице миграций 'migration_rates'.
...
# ### Передача инфекции
# Инфецированный агент может заразить заразить восприимчивых в своём городе
# Количество заражений определенно коэф-ом заразности β.
...
# ### Выздоровление или смерть
# Если агент болеет дольше, чем время "infection_period", то он с вероятностью зависящей от "death_rate" умирает, или с обратной - выздоравливает.
